(function (f, r) {
    function ya() {
        ...
        f.onresize = ka;
        ka();
        f.requestAnimationFrame ? f.requestAnimationFrame(la) : setInterval(ba, 1000 / 60);
        setInterval(G, 40);
        ma(r('#region').val());
        r('#overlays').show();
    }
    ...
    function ia() {
        ...
        r.get('http://m.agar.io/info', function (a) {
            ...
            for (c in b)
                r('#region option[value="' + c + '"]').text(S[c] + ' (' + b[c] + ' players)');
        }, 'json');
    }
    function na() {
        r('#adsBottom').hide();
    ...
    function Da(a) {
        ...
        c += 4;
        for (e = 0; e < k; e++)
            d = a.getUint32(c, !0), c += 4, w[d] && (w[d].updateCode = b);
        for (e = 0; e < q.length; e++)
            q[e].updateCode != b && q[e--].destroy();
        da && 0 == g.length && r('#overlays').fadeIn(3000);
    }
    function G() {
        if (ea()) {
            var a = O - p / 2, b = P - m / 2;
    ...
    function Ea() {
        if (0 != g.length) {
            for (var a = 0, b = 0; b < g.length; b++)
                a += g[b].size;
            a = Math.pow(Math.min(64 / a, 1), 0.4) * Math.max(m / 1080, p / 1920) / window.agarZoom;
            h = (9 * h + a) / 10;
        }
    }
    function ba() {
        ...
        D = Math.max(D, Ga());
        0 != D && (null == X && (X = new Y(24, '#FFFFFF')), X.setValue('Score: ' + ~~(D / 100)), c = X.render(), b = c.width, e.globalAlpha = 0.2, e.fillStyle = '#000000', e.fillRect(10, m - 10 - 24 - 10, b + 10, 34), e.globalAlpha = 1, e.drawImage(c, 15, m - 10 - 24 - 5));
        Ha();
        a = +new Date() - a;
        a > 1000 / 60 ? v -= 0.01 : a < 1000 / 65 && (v += 0.01);
        0.4 > v && (v = 0.4);
        1 < v && (v = 1);
    }
    function Ha() {
    ...
    function ra() {
        ...
        if (null != u || 0 != y.length)
            if (null != u || Z) {
                ...
                var a = x.getContext('2d'), b = 60, b = null == u ? b + 24 * y.length : b + 180, c = Math.min(200, 0.3 * p) / 200;
                x.width = 200 * c;
                x.height = b * c;
                a.scale(c, c);
                a.globalAlpha = 0.4;
                ...
                if (null == u)
                    ...
                    for (b = c = 0; b < u.length; ++b)
                        angEnd = c + u[b] * Math.PI * 2, a.fillStyle = Ia[b + 1], a.beginPath(), a.moveTo(100, 140), a.arc(100, 140, 80, c, angEnd, !1), a.fill(), c = angEnd;
            }
    }
    ...
    if ('agar.io' != f.location.hostname && 'localhost' != f.location.hostname && '10.10.2.13' != f.location.hostname)
        f.location = 'http://agar.io/';
    else if (f.top != f)
        f.top.location = 'http://agar.io/';
    else {
        var $, e, z, p, m, H = null, l = null, s = 0, t = 0, B = [], g = [], w = {}, q = [], C = [], y = [], O = 0, P = 0, Q = -1, R = -1, Fa = 0, E = 0, N = null, T = 0, U = 0, V = 10000, W = 10000, h = 1, I = null, va = !0, Z = !0, ha = !1, da = !1, D = 0, fa = !1, wa = !1, K = s = ~~((T + V) / 2), L = t = ~~((U + W) / 2), M = 1, J = '', u = null, Ia = [
                '#333333',
                '#FF3333',
                '#33FF33',
                '#3333FF'
            ], ja = 'ontouchstart' in f && /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent), ga = new Image();
        ...
        if ('undefined' == typeof console || 'undefined' == typeof DataView || 'undefined' == typeof WebSocket || null == xa || null == xa.getContext)
            ...
            sa.prototype = {
                ...
                setName: function (a) {
                    if (this.name = a)
                        null == this.nameCache ? this.nameCache = new Y(this.getNameSize(), '#FFFFFF', !0, '#000000') : this.nameCache.setSize(this.getNameSize()), this.nameCache.setValue(this.name);
                },
                ...
                shouldRender: function () {
                    return this.x + this.size + 40 < s - p / 2 / h || this.y + this.size + 40 < t - m / 2 / h || this.x - this.size - 40 > s + p / 2 / h || this.y - this.size - 40 > t + m / 2 / h ? !1 : !0;
                },
                draw: function () {
                    if (this.shouldRender()) {
                        ...
                        if (a)
                            e.beginPath(), e.arc(this.x, this.y, this.size, 0, 2 * Math.PI, !1);
                        else {
                            this.movePoints();
                            e.beginPath();
                        ...
                        (ha || 15 < this.size) && !a && (e.strokeStyle = '#000000', e.globalAlpha *= 0.1, e.stroke());
                        e.globalAlpha = 1;
                        ...
                        if ((Z || c) && this.name && this.nameCache) {
                            ...
                            b = Math.ceil(10 * h) / 10;
                            d.setScale(b);
                            var d = d.render(), f = ~~(d.width / b), k = ~~(d.height / b);
            ...
            Y.prototype = {
                ...
                render: function () {
                    ...
                    if (this._dirty) {
                        ...
                        b.font = f;
                        var h = b.measureText(c).width, g = ~~(0.2 * e);
                        a.width = (h + 6) * d;
                        a.height = (e + g) * d;
                        b.font = f;
                        b.scale(d, d);
