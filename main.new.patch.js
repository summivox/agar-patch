(function (g, m) {
    function Wa() {
        ...
        g.onresize = Ea;
        g.requestAnimationFrame ? g.requestAnimationFrame(Fa) : setInterval(ra, 1000 / 60);
        setInterval(N, 40);
        w && m('#region').val(w);
        Ga();
        W(m('#region').val());
            ...
            for (l = 0; l < v.length; l++)
                if (h = v[l], h.I() && !(20 >= h.size * k))
                    for (a = 0; a < h.a.length; ++a)
                        b = h.a[a].x, c = h.a[a].y, b < t - q / 2 / k || c < u - r / 2 / k || b > t + q / 2 / k || c > u + r / 2 / k || O.i(h.a[a]);
        }
    }
    function qa() {
        Y = (U - q / 2) / k + t;
    ...
    function Aa() {
        ...
        m.get(aa + '//m.agar.io/info', function (a) {
            ...
            for (c in b)
                m('#region option[value="' + c + '"]').text($[c] + ' (' + b[c] + ' players)');
        }, 'json');
    }
    function Ha() {
        m('#adsBottom').hide();
    ...
    function Da(a) {
        F = null;
        Za();
        m('#overlays').fadeIn(a ? 200 : 3000);
        a || m('#adsBottom').fadeIn(3000);
    }
    function Ga() {
    ...
    function Za() {
        ba && (ba = !1, setTimeout(function () {
            ba = !0;
        }, 60000 * Ia), g.googletag && g.googletag.pubads && g.googletag.pubads().refresh([g.mainAd]));
    }
    function Ja() {
    ...
    function cb(a) {
        ...
        case 49:
            if (null != y)
                break;
            var d = a.getUint32(c, !0), c = c + 4;
            B = [];
            for (var e = 0; e < d; ++e) {
                var l = a.getUint32(c, !0), c = c + 4;
                B.push({
                    id: l,
                    name: b()
                });
    ...
    function db(a, b) {
        ...
        b += 2;
        for (var e = 0; e < d; ++e) {
            var l = A[a.getUint32(b, !0)], h = A[a.getUint32(b + 4, !0)];
            b += 8;
        ...
        for (e = 0;;) {
            ...
            for (var g = a.getUint8(b++), k = a.getUint8(b++), q = a.getUint8(b++), g = (g << 16 | k << 8 | q).toString(16); 6 > g.length;)
                g = '0' + g;
            var g = '#' + g, k = a.getUint8(b++), q = !!(k & 1), s = !!(k & 16);
            k & 2 && (b += 4);
            k & 4 && (b += 8);
            k & 8 && (b += 16);
            for (var r, n = '';;) {
            ...
            n.W = k;
            r && n.Z(r);
            -1 != G.indexOf(d) && -1 == p.indexOf(n) && (document.getElementById('overlays').style.display = 'none', p.push(n), 1 == p.length && (t = n.x, u = n.y));
        }
        c = a.getUint32(b, !0);
        b += 4;
        for (e = 0; e < c; e++)
            d = a.getUint32(b, !0), b += 4, n = A[d], null != n && n.S();
        va && 0 == p.length && Da(!1);
    }
    function N() {
        var a;
        if (wa()) {
            a = U - q / 2;
            var b = V - r / 2;
            64 > a * a + b * b || 0.01 > Math.abs(Oa - Y) && 0.01 > Math.abs(Pa - Z) || (Oa = Y, Pa = Z, a = K(21), a.setUint8(0, 16), a.setFloat64(1, Y, !0), a.setFloat64(9, Z, !0), a.setUint32(17, 0, !0), L(a));
        }
    }
    function Ma() {
        if (wa() && null != F) {
    ...
    function eb() {
        if (0 != p.length) {
            for (var a = 0, b = 0; b < p.length; b++)
                a += p[b].size;
            a = Math.pow(Math.min(64 / a, 1), 0.4) * Qa();
            k = (9 * k + a) / 10;
        }
    }
    function ra() {
        ...
        if (0 < p.length) {
            eb();
            for (var c = a = 0, d = 0; d < p.length; d++)
                p[d].K(), a += p[d].x / p.length, c += p[d].y / p.length;
            Q = a;
            R = c;
            S = k;
            t = (t + a) / 2;
            ...
            t = (29 * t + Q) / 30, u = (29 * u + R) / 30, k = (9 * k + S * Qa()) / 10;
        Xa();
        qa();
        xa || f.clearRect(0, 0, q, r);
        xa ? (f.fillStyle = ka ? '#111111' : '#F2FBFF', f.globalAlpha = 0.05, f.fillRect(0, 0, q, r), f.globalAlpha = 1) : gb();
        v.sort(function (a, b) {
        ...
        if (ua) {
            ...
            for (d = 0; d < p.length; d++)
                f.moveTo(p[d].x, p[d].y), f.lineTo(ea, fa);
            f.stroke();
            f.restore();
        }
        f.restore();
        ...
        J = Math.max(J, hb());
        0 != J && (null == la && (la = new ma(24, '#FFFFFF')), la.u('Score: ' + ~~(J / 100)), c = la.G(), a = c.width, f.globalAlpha = 0.2, f.fillStyle = '#000000', f.fillRect(10, r - 10 - 24 - 10, a + 10, 34), f.globalAlpha = 1, f.drawImage(c, 15, r - 10 - 24 - 5));
        ib();
        b = Date.now() - b;
        b > 1000 / 60 ? z -= 0.01 : b < 1000 / 65 && (z += 0.01);
        0.4 > z && (z = 0.4);
        1 < z && (z = 1);
    }
    function gb() {
        ...
        f.globalAlpha = 0.2;
        f.scale(k, k);
        for (var a = q / k, b = r / k, c = -0.5 + (-t + a / 2) % 50; c < a; c += 50)
            f.beginPath(), f.moveTo(c, 0), f.lineTo(c, b), f.stroke();
        for (c = -0.5 + (-u + b / 2) % 50; c < b; c += 50)
    ...
    function La() {
        ...
        if (null != y || 0 != B.length)
            if (null != y || na) {
                ...
                var a = x.getContext('2d'), b = 60, b = null == y ? b + 24 * B.length : b + 180, c = Math.min(200, 0.3 * q) / 200;
                x.width = 200 * c;
                x.height = b * c;
                a.scale(c, c);
                a.globalAlpha = 0.4;
                ...
                if (null == y)
                    for (a.font = '20px Ubuntu', b = 0; b < B.length; ++b)
                        c = B[b].name || 'An unnamed cell', na || (c = 'An unnamed cell'), -1 != G.indexOf(B[b].id) ? (p[0].name && (c = p[0].name), a.fillStyle = '#FFAAAA') : a.fillStyle = '#FFFFFF', c = b + 1 + '. ' + c, a.fillText(c, 100 - a.measureText(c).width / 2, 70 + 24 * b);
                else
    ...
    var aa = g.location.protocol, $a = 'https:' == aa;
    if (g.location.ancestorOrigins && g.location.ancestorOrigins.length && 'https://apps.facebook.com' != g.location.ancestorOrigins[0])
        ...
        var pa, f, C, q, r, O = null, s = null, t = 0, u = 0, G = [], p = [], A = {}, v = [], I = [], B = [], U = 0, V = 0, Y = -1, Z = -1, fb = 0, H = 0, F = null, ga = 0, ha = 0, ia = 10000, ja = 10000, k = 1, w = null, Ra = !0, na = !0, za = !1, va = !1, J = 0, ka = !1, Sa = !1, Q = t = ~~((ga + ia) / 2), R = u = ~~((ha + ja) / 2), S = 1, P = '', y = null, oa = !1, ua = !1, sa = 0, ta = 0, ea = 0, fa = 0, Ta = 0, jb = [
                '#333333',
                '#FF3333',
                '#33FF33',
                '#3333FF'
            ], xa = !1, ca = !1, E = 1, Ba = 'ontouchstart' in g && /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent), ya = new Image();
        ...
        if ('undefined' == typeof console || 'undefined' == typeof DataView || 'undefined' == typeof WebSocket || null == Ua || null == Ua.getContext || null == g.localStorage)
            ...
            m.get(aa + '//gc.agar.io', function (a) {
                ...
                a = b[0];
                b = b[1] || '';
                -1 == ['UA'].indexOf(a) && Va.push('ussr');
                if ('AL AT BY BE BA BG HR CY CZ DK EE FI FR DE GR HU IS GB IE IT LV LT LU MK MT MD NL NO PL PT RO RU RS SK SI ES SE CH TR UA UK'.split(' ').indexOf(a) != -1) {
            ...
            g.connect = Ka;
            var da = 500, Oa = -1, Pa = -1, x = null, z = 1, la = null, M = {}, Va = 'poland;usa;china;russia;canada;australia;spain;brazil;germany;ukraine;france;sweden;chaplin;north korea;south korea;japan;united kingdom;earth;greece;latvia;lithuania;estonia;finland;norway;cia;maldivas;austria;nigeria;reddit;yaranaika;confederate;9gag;indiana;4chan;italy;bulgaria;tumblr;2ch.hk;hong kong;portugal;jamaica;german empire;mexico;sanik;switzerland;croatia;chile;indonesia;bangladesh;thailand;iran;iraq;peru;moon;botswana;bosnia;netherlands;european union;taiwan;pakistan;hungary;satanist;qing dynasty;matriarchy;patriarchy;feminism;ireland;texas;facepunch;prodota;cambodia;steam;piccolo;ea;india;kc;denmark;quebec;ayy lmao;sealand;bait;tsarist russia;origin;vinesauce;stalin;belgium;luxembourg;stussy;prussia;8ch;argentina;scotland;sir;romania;belarus;wojak;doge;nasa;byzantium;imperial japan;french kingdom;somalia;turkey;mars;pokerface;8;irs;receita federal;facebook'.split(';'), kb = [
                    '8',
                    'nasa'
                ], lb = ['m\'blob'];
            Na.prototype = {
                ...
                Z: function (a) {
                    if (this.name = a)
                        null == this.k ? this.k = new ma(this.h(), '#FFFFFF', !0, '#000000') : this.k.H(this.h()), this.k.u(this.name);
                },
                R: function () {
                    ...
                    for (; this.a.length < a;) {
                        var b = ~~(Math.random() * this.a.length), c = this.a[b];
                        this.a.splice(b, 0, {
                            Q: this,
                            e: c.e,
                            x: c.x,
                ...
                ha: function () {
                    ...
                    for (var h = this, g = this.d ? 0 : (this.id / 1000 + H / 10000) % (2 * Math.PI), d = 0; d < c; ++d) {
                        var f = a[d].e, e = a[(d - 1 + c) % c].e, l = a[(d + 1) % c].e;
                        if (15 < this.size && null != O && 20 < this.size * k && 0 != this.id) {
                            var m = !1, p = a[d].x, q = a[d].y;
                ...
                T: function (a) {
                    if (this.I()) {
                        ...
                        (za || 15 < this.size) && !b && (a.strokeStyle = '#000000', a.globalAlpha *= 0.1, a.stroke());
                        a.globalAlpha = 1;
                        ...
                        if (0 != this.id) {
                            ...
                            if ((na || c) && this.name && this.k && (null == e || -1 == kb.indexOf(d))) {
                                ...
                                d = Math.ceil(10 * k) / 10;
                                e.$(d);
                                var e = e.G(), l = ~~(e.width / d), h = ~~(e.height / d);
                                a.drawImage(e, ~~this.x - ~~(l / 2), b - ~~(h / 2), l, h);
                                b += e.height / 2 / d + 4;
                            }
                            Sa && (c || 0 == p.length && (!this.d || this.j) && 20 < this.size) && (null == this.J && (this.J = new ma(this.h() / 2, '#FFFFFF', !0, '#000000')), c = this.J, c.H(this.h() / 2), c.u(~~(this.size * this.size / 100)), d = Math.ceil(10 * k) / 10, c.$(d), e = c.G(), l = ~~(e.width / d), h = ~~(e.height / d), a.drawImage(e, ~~this.x - ~~(l / 2), b - ~~(h / 2), l, h));
                        }
                        a.restore();
                    }
                }
